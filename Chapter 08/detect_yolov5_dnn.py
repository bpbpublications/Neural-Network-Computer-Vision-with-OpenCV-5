...
def process_detection(image, yolo_shape, detection):
...
def detect_coco80objects_using_opencvdnn(impath, confidence_threshold = 0.5):
    ...
    # Load Yolo
    model = cv2.dnn.readNet("../weights/8/yolo/YOLOv5s.onnx")
    ...
    for inx in range(number_of_detections):
        ...
        if class_confidence > confidence_threshold:
            object_location = process_detection(image, yolo_shape, one_detection)
    ...
    indexes = cv2.dnn.NMSBoxes(object_coordinates, object_confidences, confidence_threshold, nms_threshold)
